...
const reg_newuser = async (req, res) => {
    ...
    try {
        ...
        if (does_exists) {
            return res.status(409).json({message: `Operation aborted — database conflict detected. Use unique or updated values and retry.`});
        }
...
const getusers = async (req, res) => {
    try {
        // Check for query parameters in the URL (e.g., /users?id=7)
        const {id, email} = req.query;
        // if an id is provided inside query
        if (id) {
            const user = await find_user_by_id(id);
            // if user is found return it, otherwise return 404
            return user ? res.status(200).json(user) : res.status(404).json({message: "User not found with associated ID!"});
        }
    ...
    try {
        ...
        if (newEmail) {
            ...
            if (!validEmail.test(newEmail)) {
                return res.status(400).json({ message: "Bad Request: Invalid or missing parameters. Please check your input and try again." });
            }
            // check conflicting matches to newEmail in existing database
            const isTaken = await conflict_check({ tableName: 'users', colName: 'email', value: newEmail, excludeID: id });
            if (isTaken) {
                return res.status(409).json({message: `Operation aborted — database conflict detected. Use unique or updated values and retry.`});
            }           
        }
        ...
        // send 200 response only when user is found with id
        if (updatedUsr) {
            res.status(200).json(updatedUsr);
        }
        else {
        // return 404 for user-not found
            return res.status(404).json({message: "User not found with associated ID!"});
        }
    } catch (err) {
        console.error(err);
        return res.status(500).json({message: 'Something went wrong on our side. Please try again later'});
...
const delete_user = async (req, res) => {
    try {
        const { id } = req.params;
        const deleted_user = await del_user_by_id(id);
        // if user is found and deleted show 200 ok else 404 no user found with associated id
        if (deleted_user) {
            res.status(200).json({message: "Successfully Deleted!", user: deleted_user});
        } else {
            return res.status(404).json({message: "Not Found: The requested resource doesn’t exist or has been removed."});
        }
    } catch (err) {
       console.error(err);
       return res.status(500).json({message: 'Something went wrong on our side. Please try again later'});
